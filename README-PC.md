...
### O que foi corrigido
- Adicionado `--publish never` no build.
- Isso impede o electron-builder de tentar publicar release no GitHub.
- Corrigido erro: `GitHub Personal Access Token is not set`.
- Adicionado `author` no `package.json` para remover aviso.

### Por que falhava
O instalador era criado, mas o electron-builder detectava CI e tentava publicar automaticamente.
Como não existia `GH_TOKEN`, ele finalizava com erro mesmo após gerar o setup.
